# Data

Thư mục `data/` lưu trữ các tệp dữ liệu trung tâm của dự án **Vietnam History & Literature Digital Library (VHLDL)**.

## index.xlsx

`index.xlsx` là cơ sở dữ liệu trung tâm của dự án, dùng để tổ chức, chuẩn hóa và liên kết các thực thể trong hệ thống.

Tệp này không thay thế nội dung nguyên bản của các tài liệu nguồn mà đóng vai trò là lớp dữ liệu giúp xây dựng các mối liên hệ giữa lịch sử, văn học và các thành phần tri thức khác của VHLDL.

Dữ liệu được bổ sung từng bước trong quá trình phát triển dự án và phục vụ cho:

- danh mục tập sách;
- tác giả;
- tác phẩm;
- nhân vật lịch sử và nhân vật văn học;
- địa danh;
- triều đại và niên đại;
- từ khóa;
- các mối liên kết giữa Lịch sử và Văn học;
- tìm kiếm, thống kê và tra cứu.

## Cấu trúc dữ liệu

Dữ liệu được tổ chức thành nhiều Sheet chuyên đề. Mỗi Sheet quản lý một nhóm dữ liệu riêng và có thể được mở rộng trong quá trình phát triển.

Các Sheet có thể bao gồm:

- Danh mục tập sách
- Tác giả
- Tác phẩm
- Nhân vật
- Địa danh
- Triều đại
- Dòng thời gian
- Từ khóa
- Liên kết dữ liệu
- Thống kê

Tên, số lượng và cấu trúc các Sheet có thể được điều chỉnh trong các phiên bản tiếp theo.

## Nguồn dữ liệu

Dữ liệu được xây dựng từ các nguồn tư liệu sử dụng trong dự án, trước hết gồm:

- **Lịch sử Việt Nam** (15 tập)
- **Tổng tập Văn học Việt Nam** (42 tập)

Trong các giai đoạn tiếp theo, cơ sở dữ liệu sẽ tiếp tục được bổ sung từ các công trình nghiên cứu và tư liệu học thuật có liên quan.

## Nguyên tắc

- Tôn trọng nguyên bản của các tài liệu nguồn.
- Không thay đổi nội dung gốc.
- Chỉ bổ sung dữ liệu mô tả, dữ liệu liên kết và dữ liệu tra cứu.
- Mọi dữ liệu đều có thể mở rộng và cập nhật trong các phiên bản tiếp theo.

## Mục đích

Thư mục `data/` là nền tảng dữ liệu của **Vietnam History & Literature Digital Library (VHLDL)**, phục vụ việc xây dựng một thư viện số có khả năng tra cứu, liên kết và nghiên cứu Lịch sử và Văn học Việt Nam.

Cấu trúc và dữ liệu sẽ được bổ sung, hoàn thiện trong suốt quá trình phát triển dự án.
